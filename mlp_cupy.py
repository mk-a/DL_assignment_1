...
def categorical_cross_entropy(pred, expect):
    ...
    return -float(cp.sum( cp.log( (pred*expect)[ (pred*expect) > 0]) ) / pred.shape[0])

def onehot(Y, m):
    ret = cp.zeros((Y.shape[0],m))
...
class MLP_2L:
    ...
    def __init__(self, input_size, n1, n2, output_size, init="normal",\
        ...
        if init == "normal":
            self.w1 = cp.random.normal(size=(n1, input_size))
            self.w2 = cp.random.normal(size=(n2, n1))
            self.w3 = cp.random.normal(size=(output_size, n2))
        elif init == "zeros":
            self.w1 = cp.zeros(shape=(n1, input_size))
            self.w2 = cp.zeros(shape=(n2, n1))
            self.w3 = cp.zeros(shape=(output_size, n2))
        elif init == "glorot":
            d1 = math.sqrt(6 / (input_size+n1))
            d2 = math.sqrt(6 / (n1 + n2))
            d3 = math.sqrt(6 / (n2 + output_size))
            self.w1 = cp.random.uniform(-d1, d1, size=(n1, input_size))
            self.w2 = cp.random.uniform(-d2, d2,size=(n2, n1))
            self.w3 = cp.random.uniform(-d3, d3,size=(output_size, n2))
        elif init=="load":
            pass
        else:
            raise ValueError("init must be 'normal', 'zeros' or 'glorot'")

        self.b1 = cp.zeros((n1, 1))
        self.b2 = cp.zeros((n2, 1))
        self.b3 = cp.zeros((output_size, 1))

    def fprop(self, X):
        self.X=X #temporary save the input value
        self.a1 = cp.dot(self.w1, X.T ) + self.b1
        self.h1 = self.activation(self.a1)

        self.a2 = cp.dot(self.w2, self.h1 ) + self.b2
        self.h2 = self.activation(self.a2)

        self.oa = cp.dot(self.w3, self.h2 ) + self.b3
    ...
    def bprop(self, Y, learning_rate):
        ...
        self.grad_w3 = cp.dot(self.grad_oa, self.h2.T) #without regularizer
        self.grad_w3 += self.l1 * self.w3 / ( cp.abs(self.w3) + (self.w3 == 0).astype(int) ) #add l1 regularizer
        self.grad_w3 += 2 * self.l2 * self.w3 # add l2 regularizer
        self.grad_b3 = self.grad_oa #without regularizer
        self.grad_b3 += self.l1 * self.b3 / ( cp.abs(self.b3) + (self.b3 == 0).astype(int) ) #add l1 regularizer
        self.grad_b3 +=  2 * self.l2 * self.b3 #add l2 regularizer

        #2nd layer
        self.grad_h2 = cp.dot(self.w3.T, self.grad_oa)
        self.grad_a2 = self.grad_h2 * self.activation.derivative(self.a2)
        self.grad_w2 = cp.dot(self.grad_a2, self.h1.T) + self.l1 * self.w2 / ( cp.abs(self.w2) + (self.w2 == 0).astype(int) ) + 2 * self.l2 * self.w2
        self.grad_b2 = self.grad_h2 + self.l1 * self.b2 / ( cp.abs(self.b2) + (self.b2 == 0).astype(int) ) + 2 * self.l2 * self.b2

        #1st layer
        self.grad_h1 = cp.dot(self.w2.T, self.grad_h2)
        self.grad_a1 = self.grad_h1 * self.activation.derivative(self.a1)
        self.grad_w1 = cp.dot(self.grad_a1, self.X) + self.l1 * self.w1 / ( cp.abs(self.w1) + (self.w1 == 0).astype(int) ) + 2 * self.l2 * self.w1
        self.grad_b1 = self.grad_h1 + self.l1 * self.b1 / ( cp.abs(self.b1) + (self.b1 == 0).astype(int) ) + 2 * self.l2 * self.b1
    ...
    def loss(self, X, Y):
        ...
        if self.l1 == 0:
            ...
            l1 = cp.sum(cp.absolute(self.w1)) + cp.sum(cp.absolute(self.b1)) + cp.sum(cp.absolute(self.w2)) + cp.sum(cp.absolute(self.b2)) + cp.sum(cp.absolute(self.w3)) + cp.sum(cp.absolute(self.b3))
        if self.l2 ==0:
            l2 = 0
        else:
            l2 = cp.sum(cp.power(self.w1,2)) + cp.sum(cp.power(self.b1,2)) + cp.sum(cp.power(self.w2,2)) + cp.sum(cp.power(self.b2,2)) + cp.sum(cp.power(self.w3,2)) + cp.sum(cp.power(self.b3, 2))
        return categorical_cross_entropy(self.fprop(X).T, onehot(Y,self.output_size)) + float(self.l1 * l1) + float(self.l2 * l2)

    def predict(self,X):
        self.fprop(X)
        return cp.argmax(self.os,axis=0)

    def evaluate(self, X, Y):
        pred = self.predict(X)
        return float(cp.sum((pred==Y).astype(int))/Y.size)
