...
class MLP:
    ...
    def __init__(self, input_size, n1, n2, output_size, c=1, init="normal", activation="linear"):
        n_parameter = n1 * (input_size + 1) + n2 * (n1 + 1) + output_size * (n2 + 1)
        print("Input dimension {:d}\tLayer 1 dimension {:d}\tLayer 2 dimension {:d}\tOutput dimension {:d}".format(input_size, n1, n2, output_size))
    ...
    def bprop(self, Y, learning_rate):
        ...
        self.grad_w2 = np.dot(self.grad_a2, self.h1.T)
        ...
        self.grad_w1 = np.dot(self.grad_a1, self.X)
